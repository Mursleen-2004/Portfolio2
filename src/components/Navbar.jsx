...
const Navbar = () => {
  ...
  return (
    <nav className="text-white px-6 py-4 flex justify-between items-center shadow-md fixed w-full top-0 z-50 ">
        ...
        {navLinks.map((item) => (
          ...
          >
            {item.label === "Contact" ? (
              <Link
                to={item.href}
                className="group py-1 flex items-center gap-2"
              >
                <img
                  src="/Contact.png"
                  alt="Message"
                  className="w-8 h-8 transition-transform duration-300 group-hover:scale-110 group-hover:drop-shadow-sm"
                />
              </Link>
            ) : (
              <a
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="absolute top-16 left-0 w-full bg-gradient-to-br items-center from-[#09303d] to-[#537895] px-6 py-4 flex flex-col gap-4 md:hidden shadow-lg z-40">
          {navLinks.map((item) =>
            item.label === "Contact" ? (
              <Link
                key={item.label}
                ...
                className="text-lg font-medium hover:text-[#FCDE59] transition-colors duration-300 flex items-center gap-2"
              >
                <img
                  src="/Contact.png"
                  alt="Message"
